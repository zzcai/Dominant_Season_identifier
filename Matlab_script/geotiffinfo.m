...
tiff_info.GeoTIFFTags = tiff_info.GeoTIFFTags(1);
% tiff_info.GeoTIFFTags
...
[idToNameMap, nameToIdMap] = map.geotiff.internal.constructGeoKeyDirectoryMap;
...
idMap = map.geotiff.internal.constructGeoKeyDirectoryMap;
...
function info = readcrs(filename, info)
% Obtain CRS info from file specified by FILENAME (full path) and add it to
% the raster referencing objected specified in info.SpatialRef. No updates
% are made in case of error.

try
    crs = map.geotiff.internal.geotiffToCRS(filename);
    if isprop(info.SpatialRef, "GeographicCRS")
        info.SpatialRef.GeographicCRS = crs;
    elseif isprop(info.SpatialRef, "ProjectedCRS")
        info.SpatialRef.ProjectedCRS = crs;
    end
catch
    % No action. Angle units might be mismatched.
end
